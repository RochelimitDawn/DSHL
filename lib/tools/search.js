...
let categoryCache;
export async function loadCategoryNames(deps, signal) {
    if (!categoryCache)
        categoryCache = new TtlCache(30 * 60 * 1000, 4);
    const cached = categoryCache.get('categories');
    if (cached)
        return cached;
    const names = new Map();
    try {
        const data = await deps.client.getJson('/categories.json', { cacheTtlMs: 30 * 60 * 1000, signal });
...
export function buildSearchTool(deps) {
    return defineTool({
        name: 'linuxdo_search',
        description: '在 Linux.do 站内搜索话题与帖子（Discourse 全文检索）。' +
