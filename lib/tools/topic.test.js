...
import { buildGetTopicTool } from './topic.js';
...
test('get_topic 全量模式：渲染楼层与深链并记录游标', async () => {
    ...
    const result = (await tool.execute({ topicId: 55 }, stubExec()));
...
test('get_topic 增量模式：无新楼返回 noNewPosts', async () => {
    ...
    const second = (await tool.execute({ topicId: 66, mode: 'incremental' }, stubExec()));
...
test('get_topic 增量模式：有新楼只返回新增部分', async () => {
    ...
    postsCount = 4;
    const incremental = (await tool.execute({ topicId: 77, mode: 'incremental' }, stubExec()));
