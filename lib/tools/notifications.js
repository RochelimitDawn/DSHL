 ...
 */
export function buildNotificationsTool(deps) {
    return defineTool({
        name: 'linuxdo_get_notifications',
        description: '读取当前登录用户在 Linux.do 的站内通知（被回复、被 @、被点赞、私信等）。' +
        ...
        async execute(args, exec) {
            ...
            const data = await deps.client.getJson('/notifications.json', { cacheTtlMs: 0, signal: exec.signal });
