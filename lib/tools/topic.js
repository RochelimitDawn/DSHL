...
import { formatPost, pruneUndefined, renderAsText } from './shared.js';
 ...
 */
export function buildGetTopicTool(deps, extras) {
    return defineTool({
        name: 'linuxdo_get_topic',
        description: '读取 Linux.do 一个话题的完整内容（按楼层）。' +
        ...
        async execute(args, exec) {
            ...
            const path = effectiveFrom !== undefined && effectiveFrom > 1
...
async function renderTopic(deps, extras, data, fromPostNumber, effectiveMaxPosts, incremental) {
    ...
    const visiblePosts = incremental || fromPostNumber > 1
