...
function makeConfig(sessionFile) {
    return {
        ...DEFAULT_CONFIG,
...
test('getJson 携带浏览器形态头与登录 cookie', async () => {
    ...
    const { impl, calls } = mockFetch(() => new Response(JSON.stringify({ ok: true }), { status: 200 }));
...
test('getJson 命中缓存时第二次请求不发出', async () => {
    const sessionFile = makeSessionFile();
    const store = new SessionStore(sessionFile);
    const { impl, calls } = mockFetch(() => new Response(JSON.stringify({ n: 1 }), { status: 200 }));
    const client = new DiscourseClient(makeConfig(sessionFile), store, impl);
...
test('Cloudflare 挑战页抛 ChallengeError', async () => {
    ...
    const { impl } = mockFetch(() => new Response('<html>Just a moment...</html>', { status: 403 }));
...
test('响应 Set-Cookie 中的 _t 续期回写会话文件', async () => {
    ...
    const store = new SessionStore(sessionFile);
    const { impl } = mockFetch(() => new Response('{}', {
        status: 200,
