...
export function createAuthorizeContext(config, redirectUri) {
...
export function extractPayload(raw) {
    ...
    if (/^[A-Za-z0-9+/=\s]+$/.test(trimmed))
        return trimmed;
    throw new Error('无法从输入中识别 payload。请粘贴浏览器地址栏中的完整回调 URL（含 ?payload=... 参数）');
}
/** 解密并校验授权 payload。 */
export function decryptAuthorizePayload(privateKeyPem, encryptedPayload, expectedNonce) {
 ...
 */
export function startCallbackServer(config) {
