...
import { decryptAuthorizePayload, extractPayload, startCallbackServer, } from './authorize.js';
...
test('extractPayload 兼容三种输入形态', () => {
    ...
    assert.equal(extractPayload(`http://127.0.0.1:54321/callback?payload=${encodeURIComponent(payload)}`), payload);
    assert.equal(extractPayload(`discourse://auth_redirect?payload=${encodeURIComponent(payload)}&extra=1`), payload);
...
test('decryptAuthorizePayload 解密并校验 nonce', () => {
    ...
    const encrypted = encryptRsaPayload(keys.publicKeyPem, JSON.stringify({ key: 'k-1', nonce }));
...
test('decryptAuthorizePayload nonce 不匹配时报错', () => {
    const keys = generateRsaKeyPair();
    const encrypted = encryptRsaPayload(keys.publicKeyPem, JSON.stringify({ key: 'k-1', nonce: 'other' }));
