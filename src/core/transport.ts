 ...
 */
export function buildCurlArgs(binary: string, req: TransportRequest): string[] {
...
export function parseCurlOutput(raw: string): { status: number; headers: Record<string, string[]>; body: string } {
  const separator = /\r?\n\r?\n/;
...
function stdoutBody(raw: string): string {
  const separator = /\r?\n\r?\n/;
  const match = separator.exec(raw);
