...
export interface StatsSources {
  clients: Array<{ name: string; client: DiscourseClient }>;
 ...
 */
export function buildStatsTool(deps: ToolDeps, sources: StatsSources) {
  return defineTool({
    name: 'linuxdo_stats',
    description:
      '查看本插件的请求统计：各站点请求次数、缓存命中率、限流排队、当前网络通道、' +
