 ...
 */
export function buildSemanticSearchTool(deps: ToolDeps) {
  return defineTool({
    name: 'linuxdo_semantic_search',
    description:
      '在 Linux.do 站内做语义搜索（基于站方 AI 向量），按含义而非关键词匹配。' +
    ...
    async execute(args, exec) {
      ...
      const data = await deps.client.getJson<Record<string, unknown>>(
