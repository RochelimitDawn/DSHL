 ...
 */
export function buildBrowseTool(deps: ToolDeps) {
  return defineTool({
    name: 'linuxdo_browse',
    description:
      '浏览 Linux.do 的话题流：latest（最新）、top（热门榜，可按日/周/月/全期）、' +
    ...
    async execute(args, exec) {
      const { stream, period, limit } = args as {
        stream: BrowseStream;
      ...
      const path =
        stream === 'top'
          ...
          : `/${stream}.json`;
      const data = await deps.client.getJson<TopicListResponse>(path, {
