...
export function buildLoginTool(deps: ToolDeps, config: Parameters<typeof startLogin>[0]) {
  return defineTool({
    name: 'linuxdo_login',
    description:
      '发起 Linux.do 授权登录。返回一个授权 URL：请把它展示给用户，' +
    ...
    async execute(_args, exec) {
      ...
      if (result.mode === 'auto') {
        return pruneUndefined({
          ...
          instruction:
            '请把 authorizeUrl 展示给用户并在浏览器打开。用户点击授权后登录将自动完成，' +
      ...
      return pruneUndefined({
        ...
        authorizeUrl: result.authorizeUrl,
        instruction:
          '本地回调端口不可用，已降级为手动模式。请让用户在浏览器打开 authorizeUrl 并授权；' +
...
export function buildLoginCompleteTool(
  deps: ToolDeps,
  client: Parameters<typeof completeLoginFromPayload>[0],
  config: Parameters<typeof completeLoginFromPayload>[1],
) {
  return defineTool({
    name: 'linuxdo_login_complete',
    description:
      '完成 Linux.do 手动授权：接收用户从浏览器地址栏复制的完整回调 URL' +
    ...
    async execute(args, exec) {
      const { callbackUrlOrPayload } = args as { callbackUrlOrPayload: string };
...
export function buildAuthStatusTool(
  deps: ToolDeps,
  client: Parameters<typeof completeLoginFromCallback>[0],
) {
  return defineTool({
    name: 'linuxdo_auth_status',
    description:
      '查询 Linux.do 登录状态。检索类工具报 AUTH_REQUIRED 错误后，' +
    ...
    async execute(_args, exec) {
      ...
      // 有 token 时用轻量接口验证有效性
      try {
        const session = await client.getJson<{ current_user?: { username?: string } }>(
