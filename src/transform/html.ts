...
function extractCodeBlocks(html: string): { stripped: string; blocks: CodePlaceholder[] } {
  const blocks: CodePlaceholder[] = [];
...
function decodeCodeBlock(inner: string): string {
 ...
 */
export function foldQuotes(html: string): string {
  return html.replace(
