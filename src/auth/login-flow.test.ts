...
function mockFetchForOtp(routes: Map<string, () => Response>) {
...
test('无 pending 会话时 completeLogin 报错', async () => {
  ...
  await assert.rejects(
    completeLoginFromCallback(client, config, 'AAAA'),
    /没有进行中的授权会话/,
  );
});
